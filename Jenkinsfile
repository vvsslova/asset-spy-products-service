pipeline {
    ...
    environment {
        DOCKER_IMAGE = "vvsslova/asset-spy-product-service"
        DOCKER_TAG = "${env.BUILD_NUMBER}"
        GITHUB_REPO = "vvsslova/asset-spy-products-service"
    ...
    stages {
        ...
        stage('Deploy to Kubernetes') {
            ...
            steps {
            	...
            	sh '''
            	kubectl set image deploy/product-service product-service=${DOCKER_IMAGE}:${DOCKER_TAG} -n asset-spy
            	kubectl rollout status deploy/product-service --timeout=180s -n asset-spy
            	'''
            	}
            }
        }
